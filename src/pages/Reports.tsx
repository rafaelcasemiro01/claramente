...
const MOOD_COLOR: Record<string,string> = { positive:'#22C55E', negative:'#EF4444', neutral:'#8B5CF6', mixed:'#F59E0B', anxious:'#6366F1' }
...
export default function Reports() {
  ...
  async function generate() {
    ...
    try {
      const res = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({ model: 'claude-sonnet-4-6', max_tokens: 1000, messages: [{ role: 'user', content: `Analise e responda SOMENTE com JSON válido, sem markdown:\n${transcript}\n\nFormato: {"dominant_mood":"positive|negative|neutral|mixed|anxious","recurring_themes":["tema1","tema2"],"progress_notes":"análise em 2-3 frases","suggestions":["s1","s2","s3"]}` }] })
      })
  ...
  const moodColor = report?.dominant_mood ? (MOOD_COLOR[report.dominant_mood] || C.accent) : C.accent
    ...
    <div style={{ minHeight: '100dvh', background: C.bg, fontFamily: "'Inter',sans-serif" }}>
      ...
      <div style={{ maxWidth: 680, margin: '0 auto', padding: '28px 20px 64px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 8, marginBottom: 24 }}>
          {PERIODS.map(p => (
            <button key={p.key} onClick={() => setPeriod(p.key)} style={{ padding: '12px 8px', borderRadius: 10, cursor: 'pointer', fontFamily: "'Inter',sans-serif", background: period === p.key ? C.accentBg : C.surface, border: `1px solid ${period === p.key ? `${C.accent}45` : C.border}`, transition: 'all 0.15s', textAlign: 'center' }}
              ...
              <p style={{ fontSize: 11, color: period === p.key ? C.accent : C.textMuted, margin: 0, opacity: 0.8 }}>{p.sub}</p>
            </button>
          ))}
        </div>
        ...
        {!loading && !report && (
          <div style={{ textAlign: 'center', padding: '60px 24px', animation: 'fIn 0.4s ease' }}>
            <div style={{ width: 52, height: 52, borderRadius: 14, background: C.accentBg, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px' }}>
            ...
            {msgCount > 0 && (
              <button onClick={generate} disabled={generating} style={{ height: 40, padding: '0 24px', borderRadius: 8, border: 'none', background: generating ? C.accentBg : C.accent, color: generating ? C.accent : '#FFF', fontSize: 14, fontWeight: 600, cursor: generating ? 'not-allowed' : 'pointer', fontFamily: "'Inter',sans-serif", transition: 'all 0.15s' }}>
        ...
        {!loading && report && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10, animation: 'fIn 0.4s ease' }}>
            ...
            <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, padding: 20, position: 'relative', overflow: 'hidden' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 12 }}>
                ...
                <div style={{ textAlign: 'right' }}>
                  ...
                  <p style={{ fontSize: 28, fontWeight: 700, color: C.accent, lineHeight: 1 }}>{report.message_count}</p>
              ...
              <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, padding: 20 }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7 }}>
                  {report.recurring_themes.map((t, i) => (
                    <span key={i} style={{ padding: '5px 12px', borderRadius: 20, fontSize: 13, fontWeight: 500, background: C.accentBg, color: C.accent, border: `1px solid ${C.accent}22` }}>{t}</span>
                  ))}
                </div>
              </div>
            )}
            ...
            {report.progress_notes && (
              <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderLeft: `3px solid ${C.accent}`, borderRadius: 12, padding: 20 }}>
