...
export default function Profile() {
  ...
  const onFocus = (e: React.FocusEvent<HTMLInputElement>) => { e.currentTarget.style.borderColor=ACCENT; e.currentTarget.style.boxShadow=`0 0 0 3px ${ACCENT}18` }
  const onBlur  = (e: React.FocusEvent<HTMLInputElement>) => { e.currentTarget.style.borderColor=C.inputBorder; e.currentTarget.style.boxShadow='none' }
  ...
  // ── Exclusão permanente via RPC (requer função SQL no Supabase) ──
  async function deleteAccount() {
    ...
    try {
      ...
      setDelErr('Erro ao excluir. Tente novamente.')
      setDeleting(false)
    }
  }
      ...
      <header style={{ background:C.surface, borderBottom:`1px solid ${C.border}`, height:56, padding:'0 24px', display:'flex', alignItems:'center', justifyContent:'space-between', position:'sticky', top:0, zIndex:10 }}>
        ...
        <button onClick={toggle} style={{ width:34, height:34, borderRadius:8, border:`1px solid ${C.border}`, background:'transparent', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center' }}>
          <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke={C.textSub} strokeWidth="2" strokeLinecap="round">
            {isDark ? <><circle cx="12" cy="12" r="4"/><line x1="12" y1="2" x2="12" y2="4"/><line x1="12" y1="20" x2="12" y2="22"/><line x1="4.22" y1="4.22" x2="5.64" y2="5.64"/><line x1="18.36" y1="18.36" x2="19.78" y2="19.78"/><line x1="2" y1="12" x2="4" y2="12"/><line x1="20" y1="12" x2="22" y2="12"/></>
              : <path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/>}
          </svg>
        </button>
      </header>

      <div style={{ maxWidth:560, margin:'0 auto', padding:'28px 20px 80px', animation:'fIn 0.35s ease' }}>

        <div style={{ display:'flex', alignItems:'center', gap:14, marginBottom:24 }}>
          <div style={{ width:52, height:52, borderRadius:'50%', background:C.accentBg, border:`1px solid ${ACCENT}30`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:20, fontWeight:700, color:ACCENT, flexShrink:0, boxShadow:`0 0 16px ${ACCENT}20` }}>
        ...
        <Section title="Informações pessoais" C={C}>
          <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
            <div>
              ...
              <input value={name} onChange={e=>setName(e.target.value)} style={inp()} onFocus={onFocus} onBlur={onBlur}/>
            </div>
            <div>
        ...
        <Section title="Alterar senha" C={C}>
          <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
            <div>
              ...
              <div style={{ position:'relative' }}>
                <input type={showPw?'text':'password'} value={newPw} onChange={e=>setNewPw(e.target.value)} placeholder="Crie uma senha forte"
                  style={inp({ paddingRight:42 })} onFocus={e=>{ onFocus(e); setPwFocus(true) }} onBlur={onBlur}/>
              ...
              {pwFocus&&newPw.length>0 && (
                <div style={{ marginTop:8, padding:12, background:C.inner, borderRadius:8, border:`1px solid ${C.border}` }}>
                  ...
                  {RULES.map(r=>{ const ok=r.ok(newPw); return (
            ...
            <div>
              ...
              <div style={{ position:'relative' }}>
                <input type={showCf?'text':'password'} value={cfPw} onChange={e=>setCfPw(e.target.value)} placeholder="Repita a nova senha"
              ...
              {cfPw.length>3&&<p style={{ fontSize:11, marginTop:4, color:cfPw===newPw?'#22C55E':'#EF4444' }}>{cfPw===newPw?'✓ Senhas coincidem':'✗ Senhas não coincidem'}</p>}
            </div>
            {pwErr  && <p style={{ fontSize:12, color:'#EF4444' }}>{pwErr}</p>}
            {pwDone && <p style={{ fontSize:12, color:'#22C55E' }}>✓ Senha alterada!</p>}
        ...
        <Section title="Dados e privacidade (LGPD)" C={C}>
          <p style={{ fontSize:13, color:C.textSub, lineHeight:1.7, marginBottom:14 }}>
            Conforme a LGPD, você tem direito ao esquecimento. Seus dados podem ser removidos permanentemente.
          </p>
        ...
        <div style={{ background:isDark?'rgba(239,68,68,0.06)':'#FFF5F5', border:`1px solid ${isDark?'rgba(239,68,68,0.2)':'#FED7D7'}`, borderRadius:12, padding:20 }}>
          ...
          {!showDel ? (
            <>
              <p style={{ fontSize:13, color:C.textSub, lineHeight:1.7, marginBottom:14 }}>
                Excluir sua conta remove <strong style={{ color:C.text }}>permanentemente</strong> todas as conversas, relatórios e seu acesso ao sistema. Esta ação não pode ser desfeita.
              </p>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
              <p style={{ fontSize:13, color:'#EF4444', fontWeight:500, lineHeight:1.6 }}>Digite <strong>EXCLUIR</strong> para confirmar a exclusão permanente:</p>
              ...
              />
              {delErr&&<p style={{ fontSize:12, color:'#EF4444' }}>{delErr}</p>}
              <div style={{ display:'flex', gap:8 }}>
                <button onClick={()=>{ setShowDel(false); setDelText(''); setDelErr('') }} style={{ height:36, padding:'0 14px', borderRadius:8, border:`1px solid ${C.border}`, background:'transparent', color:C.textSub, fontSize:13, fontWeight:500, cursor:'pointer', fontFamily:"'Inter',sans-serif" }}>Cancelar</button>
