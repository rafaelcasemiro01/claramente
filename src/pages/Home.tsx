...
// ─── Aura azul atmosférica (exclusiva do Home) ─────────────────
function FooterAura() {
  return (
    <div style={{
      position: 'fixed', bottom: 0, left: 0, right: 0,
      height: 110, zIndex: 0, pointerEvents: 'none', overflow: 'hidden',
    }}>
      <div style={{
        position: 'absolute', inset: 0,
        background: 'linear-gradient(to top, rgba(37,99,235,0.22) 0%, rgba(59,130,246,0.13) 35%, rgba(96,165,250,0.05) 65%, transparent 100%)',
      }} />
      <div style={{
        position: 'absolute', bottom: 0, left: '50%',
        transform: 'translateX(-50%)',
        width: '120%', height: 70,
        background: 'radial-gradient(ellipse at 50% 100%, rgba(96,165,250,0.28) 0%, rgba(59,130,246,0.12) 45%, transparent 70%)',
        animation: 'auraBreath 5s ease-in-out infinite',
      }} />
      <div style={{
        position: 'absolute', bottom: 0, left: 0, right: 0, height: 3,
        background: 'linear-gradient(90deg, transparent 0%, rgba(59,130,246,0) 5%, rgba(96,165,250,1) 30%, rgba(147,197,253,1) 50%, rgba(96,165,250,1) 70%, rgba(59,130,246,0) 95%, transparent 100%)',
        backgroundSize: '200% 100%',
        animation: 'footerBeam 4s linear infinite',
        boxShadow: '0 0 18px rgba(96,165,250,0.7), 0 0 36px rgba(59,130,246,0.35)',
      }} />
    </div>
  )
}

// ─── Bot avatar com nova logo C-3PO ────────────────────────────
function BotAvatar({ size = 28, accent }: { size?: number; accent: string }) {
  return (
    <div style={{
      ...
      boxShadow: `0 0 10px ${accent}1E`,
    }}>
      <ClaramenteLogo size={size * 0.8} accent={accent} />
    </div>
  )
}
...
// ─── Action Card ───────────────────────────────────────────────
function ActionCard({
  title, subtitle, Icon, accent = false, isDark, accentColor, onClick,
}: {
  ...
  return (
    <button
      ...
      style={{
        ...
        background: accent
          ? (hov ? `${accentColor}1A` : `${accentColor}0E`)
          : (hov ? (isDark ? '#1E293B' : '#F0F9FF') : (isDark ? '#1E293B' : '#FFFFFF')),
        border: `1px solid ${accent
          ? (hov ? `${accentColor}55` : `${accentColor}30`)
          : (hov ? (isDark ? '#334155' : '#BFDBFE') : (isDark ? '#1E293B' : '#E0F2FE'))}`,
        transition: 'all 0.15s ease',
        transform: hov ? 'translateY(-1px)' : 'translateY(0)',
        boxShadow: hov
          ? (accent
            ? `0 4px 16px ${accentColor}22`
            : (isDark ? '0 4px 16px rgba(0,0,0,0.3)' : '0 4px 16px rgba(37,99,235,0.1)'))
          : 'none',
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
        ...
        <p style={{
          ...
          color: accent ? accentColor : (isDark ? '#F1F5F9' : '#0F172A'),
      ...
      <p style={{
        fontSize: 12, margin: 0, color: '#64748B',
...
function Typewriter({ text, speed = 35, onDone }: { text: string; speed?: number; onDone?: () => void }) {
  ...
  return (
    <>
      {shown}
      {!done && <span style={{ opacity: 0.3, animation: 'blink 1s infinite' }}>|</span>}
    </>
  )
}
...
export default function Home() {
  ...
  // ─── Tokens de cor (100% azul, sem roxo) ──────────────────
  const C = isDark ? {
    bg:          '#0F172A',
    surface:     '#1E293B',
    border:      '#334155',
  ...
  const CSS = `
    ...
    *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
    ...
    @keyframes auraBreath { 0%,100%{opacity:0.75;transform:translateX(-50%) scaleX(1)} 50%{opacity:1;transform:translateX(-50%) scaleX(1.06)} }
    ...
    return (
      <aside style={{
        ...
        <div style={{ padding: '16px 16px 12px', borderBottom: `1px solid ${C.border}`, flexShrink: 0 }}>
          ...
          <button
            onClick={newChat}
            style={{ width: '100%', height: 36, borderRadius: 8, border: 'none', background: ACCENT, color: '#FFF', fontSize: 13, fontWeight: 600, fontFamily: "'Inter',sans-serif", cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7, transition: 'filter 0.15s', WebkitTapHighlightColor: 'transparent' }}
              ...
              {g.items.map(c => {
                ...
                return (
                  <button
                    ...
                    style={{ width: '100%', textAlign: 'left', padding: '9px 10px', borderRadius: 8, border: 'none', cursor: 'pointer', background: active ? C.accentBg : 'transparent', display: 'block', marginBottom: 1, transition: 'all 0.12s', WebkitTapHighlightColor: 'transparent', minHeight: 40 }}
                    ...
                    <p style={{ fontSize: 12, color: C.textMuted, margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontFamily: "'Inter',sans-serif" }}>
                      {c.preview}
                    </p>
                  </button>
                )
        ...
        <div style={{ padding: '8px', borderTop: `1px solid ${C.border}` }}>
          <button
            onClick={journal}
            style={{ width: '100%', padding: '9px 10px', borderRadius: 8, border: `1px solid ${C.border}`, background: 'transparent', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8, color: C.textSub, fontSize: 13, fontWeight: 500, fontFamily: "'Inter',sans-serif", transition: 'all 0.12s', WebkitTapHighlightColor: 'transparent' }}
        ...
        <div style={{ height: 56, padding: '0 12px', borderTop: `1px solid ${C.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-around' }}>
          {sidebarIcons.map(({ I, fn, tip }) => (
            <button
              ...
              style={{ width: 36, height: 36, borderRadius: 8, border: `1px solid ${C.border}`, background: 'transparent', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.15s', WebkitTapHighlightColor: 'transparent' }}
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
          ...
          <div style={{ maxWidth: 680, margin: '0 auto', display: 'flex', flexDirection: 'column' }}>
            ...
            {messages.length === 0 && (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '40px 0 24px', animation: 'fIn 0.4s ease' }}>
                ...
                {/* Logo C-3PO grande na welcome screen */}
                <div style={{ marginBottom: 20, filter: `drop-shadow(0 0 20px ${ACCENT}45)` }}>
                  <ClaramenteLogo size={68} accent={ACCENT} />
                </div>
                ...
                {smartSummary && !loadingSmartSummary && (
                  <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, padding: '12px 16px', marginTop: 20, width: '100%', animation: 'fIn 0.4s ease' }}>
                    <p style={{ fontSize: 14, color: C.textSub, lineHeight: 1.7, margin: 0, fontFamily: "'Inter',sans-serif", fontStyle: 'italic' }}>
                      {smartSummary}
                    </p>
                  </div>
                )}
                ...
                {greetDone && (
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gridAutoRows: '1fr', gap: 10, marginTop: 20, width: '100%', animation: 'fIn 0.4s ease 0.1s both' }}>
                    <ActionCard
                      title="Journaling guiado" subtitle="Sessão reflexiva profunda"
                      Icon={Sparkle} accent accentColor={ACCENT} isDark={isDark}
                      onClick={() => { audio.init(); journal() }}
                    />
                    <ActionCard
                      title="Estou ansioso" subtitle="Preciso conversar sobre isso"
                      accentColor={ACCENT} isDark={isDark}
                      onClick={() => { audio.init(); sendMessage('Estou me sentindo ansioso ultimamente.') }}
                    />
                    <ActionCard
                      title="Quero refletir" subtitle="Momento de introspecção"
                      accentColor={ACCENT} isDark={isDark}
                      onClick={() => { audio.init(); sendMessage('Quero fazer uma reflexão sobre minha vida.') }}
                    />
                    <ActionCard
                      title="Me sinto bem" subtitle="Compartilhar gratidão"
                      accentColor={ACCENT} isDark={isDark}
                      onClick={() => { audio.init(); sendMessage('Estou me sentindo bem hoje!') }}
                    />
                  </div>
                )}
              </div>
            ...
            {messages.map(msg => (
              <div key={msg.id} style={{ display: 'flex', flexDirection: 'column', marginBottom: 4, animation: 'mIn 0.25s ease' }}>
                {msg.role === 'user' ? (
                  ...
                  /* Mensagem da IA — usa nova logo C-3PO */
        ...
        <div style={{ padding: '12px 20px', paddingBottom: 'max(12px,env(safe-area-inset-bottom,12px))', background: C.bg, borderTop: `1px solid ${C.border}`, flexShrink: 0, position: 'relative', zIndex: 1 }}>
          <div style={{ maxWidth: 680, margin: '0 auto' }}>
            <div
              style={{ display: 'flex', gap: 8, alignItems: 'flex-end', background: C.inputBg, borderRadius: 14, padding: '8px', border: `1px solid ${C.inputBorder}`, boxShadow: isDark ? '0 1px 4px rgba(0,0,0,0.3)' : '0 1px 4px rgba(37,99,235,0.06)', transition: 'border-color 0.15s, box-shadow 0.15s' }}
              onFocusCapture={e => {
              ...
              onBlurCapture={e => {
                ...
                e.currentTarget.style.boxShadow   = isDark ? '0 1px 4px rgba(0,0,0,0.3)' : '0 1px 4px rgba(37,99,235,0.06)'
              }}
            >
              <textarea
                ref={taRef}
              ...
              <button
                ...
                style={{ width: 38, height: 38, borderRadius: 10, border: 'none', flexShrink: 0, cursor: isTyping || !input.trim() ? 'not-allowed' : 'pointer', background: isTyping || !input.trim() ? C.btnBg : ACCENT, display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.15s', WebkitTapHighlightColor: 'transparent' }}
