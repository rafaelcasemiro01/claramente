...
function BotAvatar({ size = 26, accent }: { size?: number; accent: string }) {
  return (
    <div style={{ width: size, height: size, borderRadius: '50%', background: `${accent}18`, border: `1px solid ${accent}28`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
...
function TypingDots({ color }: { color: string }) {
  return (
    <div style={{ display: 'flex', gap: 4, alignItems: 'center', padding: '4px 0' }}>
      {[0, 1, 2].map(i => (
        <div key={i} style={{ width: 6, height: 6, borderRadius: '50%', background: color, opacity: 0.5, animation: `tdot 1.3s ${i * 0.18}s ease-in-out infinite` }} />
      ))}
    </div>
  )
}
  ...
  return (
    <button
      ...
      style={{
        ...
        gap: 4,
        background: accent
          ? (hov ? `${accentColor}16` : `${accentColor}0C`)
          : (hov ? (isDark ? '#222' : '#FAFAFA') : (isDark ? '#1A1A1A' : '#FFFFFF')),
        border: `1px solid ${accent
          ? (hov ? `${accentColor}50` : `${accentColor}28`)
          : (hov ? (isDark ? '#383838' : '#D0D0D0') : (isDark ? '#272727' : '#E8E8E8'))}`,
        transition: 'all 0.15s ease',
        transform: hov ? 'translateY(-1px)' : 'translateY(0)',
        boxShadow: hov ? (isDark ? '0 4px 16px rgba(0,0,0,0.3)' : '0 4px 16px rgba(0,0,0,0.07)') : 'none',
        WebkitTapHighlightColor: 'transparent',
      }}
    >
      {/* ícone alinhado ao título na mesma linha */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
        {Icon && <Icon size={14} color={accent ? accentColor : (isDark ? '#A0A0A0' : '#6B6B6B')} />}
        <p style={{ fontSize: 13, fontWeight: 600, margin: 0, color: accent ? accentColor : (isDark ? '#F5F5F5' : '#0F0F0F'), fontFamily: "'Inter',sans-serif", lineHeight: 1.3 }}>
          {title}
        </p>
      </div>
      <p style={{ fontSize: 12, fontWeight: 400, margin: 0, color: isDark ? '#606060' : '#9B9B9B', fontFamily: "'Inter',sans-serif", lineHeight: 1.4, paddingLeft: Icon ? 21 : 0 }}>
        {subtitle}
      </p>
    </button>
  )
...
export default function Home() {
  ...
  const C = isDark ? {
    bg: '#111111', surface: '#1A1A1A', border: '#272727', sidebar: '#161616',
    text: '#F5F5F5', textSub: '#A0A0A0', textMuted: '#606060',
    accentBg: '#1E1535', userBubble: '#1E1535', userBorder: '#3D2B6B', userText: '#DDD6FE',
    inputBg: '#1A1A1A', inputBorder: '#2E2E2E', placeholder: '#404040', btnBg: '#272727',
  } : {
    bg: '#F7F7F8', surface: '#FFFFFF', border: '#E8E8E8', sidebar: '#FFFFFF',
    text: '#0F0F0F', textSub: '#6B6B6B', textMuted: '#9B9B9B',
    accentBg: '#F4F0FF', userBubble: '#F4F0FF', userBorder: '#E2D9FF', userText: '#4C1D95',
    inputBg: '#FFFFFF', inputBorder: '#E2E2E2', placeholder: '#ACACAC', btnBg: '#F4F4F5',
  }
  ...
  function Sidebar() {
    return (
      <aside style={{ width: 256, flexShrink: 0, height: '100%', display: 'flex', flexDirection: 'column', background: C.sidebar, borderRight: `1px solid ${C.border}` }}>
        ...
        <div style={{ padding: '16px 16px 12px', borderBottom: `1px solid ${C.border}`, flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 9, marginBottom: 12 }}>
            ...
            <span style={{ fontSize: 15, fontWeight: 700, color: C.text, fontFamily: "'Inter',sans-serif", letterSpacing: -0.3 }}>Claramente</span>
          </div>
        ...
        <div style={{ padding: '8px', borderTop: `1px solid ${C.border}` }}>
          ...
          <button onClick={() => setHudOpen(p => !p)} style={{ width: '100%', padding: '7px 10px', borderRadius: 8, border: 'none', background: 'transparent', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between', color: C.textMuted, fontSize: 11, fontWeight: 600, fontFamily: "'Inter',sans-serif", letterSpacing: 0.5, textTransform: 'uppercase', marginTop: 2, WebkitTapHighlightColor: 'transparent' }}
            ...
            <span>Diagnóstico</span><span>{hudOpen ? '▲' : '▼'}</span>
          </button>
        ...
        <div style={{ padding: '12px 14px', borderTop: `1px solid ${C.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
            <div style={{ width: 30, height: 30, borderRadius: '50%', background: C.accentBg, border: `1px solid ${ACCENT}25`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 700, color: ACCENT, flexShrink: 0 }}>
              {firstName.charAt(0).toUpperCase()}
            </div>
            <div>
              <p style={{ fontSize: 13, fontWeight: 600, color: C.text, margin: 0, fontFamily: "'Inter',sans-serif" }}>{firstName}</p>
              <p style={{ fontSize: 11, color: C.textMuted, margin: 0, fontFamily: "'Inter',sans-serif" }}>Plano gratuito</p>
            </div>
          </div>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
          ...
          <div style={{ maxWidth: 680, margin: '0 auto', display: 'flex', flexDirection: 'column' }}>
            ...
            {messages.length === 0 && (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '40px 0 24px', animation: 'fIn 0.4s ease' }}>
                ...
                <h2 style={{ fontFamily: "'Inter',sans-serif", fontSize: 'clamp(20px,4vw,26px)', fontWeight: 700, color: C.text, marginBottom: 8, textAlign: 'center', letterSpacing: -0.5 }}>
                  {!greetDone
                    ? <Typewriter text={`Olá, ${firstName}`} speed={45} onDone={() => setGreetDone(true)} />
                    : `Olá, ${firstName}`}
                </h2>

                {loadingSmartSummary && <div style={{ marginBottom: 20 }}><TypingDots color={ACCENT} /></div>}

                {smartSummary && !loadingSmartSummary && (
                  <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, padding: '12px 16px', marginBottom: 24, width: '100%', animation: 'fIn 0.4s ease' }}>
                ...
                {!smartSummary && !loadingSmartSummary && greetDone && (
                  <p style={{ fontSize: 14, color: C.textSub, textAlign: 'center', marginBottom: 4, lineHeight: 1.7, fontFamily: "'Inter',sans-serif" }}>
                    Como posso te ajudar hoje?
                  </p>
                )}

                {/* Cards 2×2 — full width, alturas iguais */}
                {greetDone && (
                  <div style={{
                    display: 'grid',
        ...
        <div style={{ padding: '12px 20px', paddingBottom: 'max(12px,env(safe-area-inset-bottom,12px))', background: C.bg, borderTop: `1px solid ${C.border}`, flexShrink: 0 }}>
          <div style={{ maxWidth: 680, margin: '0 auto' }}>
            <div style={{ display: 'flex', gap: 8, alignItems: 'flex-end', background: C.inputBg, borderRadius: 14, padding: '8px', border: `1px solid ${recording ? 'rgba(239,68,68,0.5)' : C.inputBorder}`, boxShadow: isDark ? '0 1px 4px rgba(0,0,0,0.3)' : '0 1px 4px rgba(0,0,0,0.06)', transition: 'border-color 0.15s, box-shadow 0.15s' }}
              onFocusCapture={e => { e.currentTarget.style.borderColor = ACCENT; e.currentTarget.style.boxShadow = `0 0 0 3px ${ACCENT}12` }}
