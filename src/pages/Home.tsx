...
function ActionCard({ title, subtitle, Icon, accent = false, isDark, accentColor, onClick }: {
  ...
  return (
    <button onClick={onClick} onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)} style={{
      ...
      <p style={{ fontSize: 12, fontWeight: 400, margin: 0, color: isDark ? '#64748B' : '#64748B', fontFamily: "'Inter',sans-serif", lineHeight: 1.4, paddingLeft: Icon ? 21 : 0 }}>{subtitle}</p>
    </button>
  )
}
...
export default function Home() {
  ...
  const C = isDark ? {
    bg: '#0F172A', surface: '#1E293B', border: '#1E293B', sidebar: '#0F172A',
    text: '#F1F5F9', textSub: '#94A3B8', textMuted: '#475569',
    accentBg: 'rgba(30,64,175,0.2)', userBubble: 'rgba(30,64,175,0.2)',
    userBorder: '#1E40AF', userText: '#BFDBFE',
    inputBg: '#1E293B', inputBorder: '#334155', placeholder: '#475569', btnBg: '#1E293B',
  } : {
    bg: '#F0F9FF', surface: '#FFFFFF', border: '#E0F2FE', sidebar: '#FFFFFF',
    text: '#0F172A', textSub: '#374151', textMuted: '#64748B',
    accentBg: '#EFF6FF', userBubble: '#EFF6FF',
    userBorder: '#BFDBFE', userText: '#1E40AF',
      ...
      <aside style={{ width:256, flexShrink:0, height:'100%', display:'flex', flexDirection:'column', background:C.sidebar, borderRight:`1px solid ${C.border}` }}>
        ...
        <div style={{ padding:'16px 16px 12px', borderBottom:`1px solid ${C.border}`, flexShrink:0 }}>
          <div style={{ display:'flex', alignItems:'center', gap:9, marginBottom:12 }}>
            <div style={{ width:30, height:30, borderRadius:8, background:C.accentBg, border:`1px solid ${isDark?'#1E40AF':'#BFDBFE'}`, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, boxShadow:`0 0 12px ${ACCENT}25` }}>
              <Crystal size={16} color={ACCENT} dim={`${ACCENT}22`} mid={`${ACCENT}35`} line={`${ACCENT}65`} />
            </div>
        ...
        <div style={{ padding:'12px 14px', borderTop:`1px solid ${C.border}`, display:'flex', alignItems:'center', justifyContent:'space-between', flexShrink:0 }}>
          <div style={{ display:'flex', alignItems:'center', gap:9 }}>
            <div style={{ width:30, height:30, borderRadius:'50%', background:C.accentBg, border:`1px solid ${ACCENT}30`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:13, fontWeight:700, color:ACCENT, flexShrink:0 }}>
          ...
          <div style={{ maxWidth:680, margin:'0 auto', display:'flex', flexDirection:'column' }}>
            ...
            {messages.length===0 && (
              <div style={{ display:'flex', flexDirection:'column', alignItems:'center', padding:'40px 0 24px', animation:'fIn 0.4s ease' }}>
                ...
                <div style={{ width:48, height:48, borderRadius:14, background:C.accentBg, border:`1px solid ${isDark?'#1E40AF':'#BFDBFE'}`, display:'flex', alignItems:'center', justifyContent:'center', marginBottom:20, boxShadow:`0 0 24px ${ACCENT}25` }}>
                  <Crystal size={26} color={ACCENT} dim={`${ACCENT}25`} mid={`${ACCENT}35`} line={`${ACCENT}65`} />
                </div>
                ...
                {!smartSummary&&!loadingSmartSummary&&greetDone && (
                  <p style={{ fontSize:14, color:C.textSub, textAlign:'center', marginBottom:4, lineHeight:1.7, fontFamily:"'Inter',sans-serif" }}>Como posso te ajudar hoje?</p>
                )}
              ...
              <div style={{ flex:1, minWidth:0 }}>
                {recording ? (
                  <div style={{ display:'flex', alignItems:'center', gap:8, padding:'8px 4px 0' }}>
                    <div style={{ display:'flex', gap:2 }}>{Array.from({length:8}).map((_,i)=><div key={i} style={{ width:2, borderRadius:2, background:'#EF4444', opacity:0.6, animation:`tdot ${0.5+(i%4)*0.12}s ${i*0.07}s infinite`, height:`${8+(i%4)*4}px` }}/>)}</div>
