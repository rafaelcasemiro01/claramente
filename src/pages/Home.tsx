...
import {
  ...
  BarChart, Person, LogOut, Sun, Moon, Menu, Crystal,
} from '@/components/Icons'
import type { ConversationItem } from '@/hooks/useChat'

// ─── Atmospheric footer aura (só no Home) ─────────────────────
function FooterAura() {
  return (
    <div style={{
...
function BotAvatar({ size = 26, accent }: { size?: number; accent: string }) {
  return (
    <div style={{ width: size, height: size, borderRadius: '50%', background: `${accent}18`, border: `1px solid ${accent}30`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
...
function ActionCard({ title, subtitle, Icon, accent = false, isDark, accentColor, onClick }: {
  ...
  return (
    <button onClick={onClick} onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)} style={{
      ...
      boxShadow: hov ? (accent ? `0 4px 16px ${accentColor}22` : (isDark ? '0 4px 16px rgba(0,0,0,0.3)' : '0 4px 16px rgba(37,99,235,0.1)')) : 'none',
...
export default function Home() {
  ...
  const C = isDark ? {
    ...
    sidebar: '#0F172A', sidebarB: '#1E293B',
    ...
    sidebar: '#FFFFFF', sidebarB: '#E0F2FE',
          ...
          <div style={{ maxWidth: 680, margin: '0 auto', display: 'flex', flexDirection: 'column' }}>
            ...
            {messages.length === 0 && (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '40px 0 24px', animation: 'fIn 0.4s ease' }}>
                ...
                <div style={{ marginBottom: 20, filter: `drop-shadow(0 0 18px ${ACCENT}40)` }}>
                  <ClaramenteLogo size={60} accent={ACCENT} />
                </div>
