...
export default function Landing() {
  ...
  const C = isDark ? {
    ...
    accentBg: 'rgba(30,64,175,0.2)', accentB: '#1E40AF',
    inputBg: '#1E293B', inputB: '#334155', tabBg: '#0F172A', ph: '#475569',
    errBg: 'rgba(239,68,68,0.1)', errB: 'rgba(239,68,68,0.3)', errT: '#FCA5A5',
    okBg:  'rgba(34,197,94,0.1)',  okB:  'rgba(34,197,94,0.3)',  okT:  '#86EFAC',
  } : {
    bg: '#F0F9FF', surface: '#FFFFFF', border: '#E0F2FE',
    text: '#0F172A', textSub: '#374151', textMuted: '#64748B',
    accentBg: '#EFF6FF', accentB: '#BFDBFE',
    inputBg: '#FFFFFF', inputB: '#E0F2FE', tabBg: '#F8FAFC', ph: '#94A3B8',
    errBg: '#FFF5F5', errB: '#FED7D7', errT: '#C53030',
    okBg:  '#F0FFF4', okB:  '#C6F6D5', okT:  '#276749',
  }
  ...
  function reset() {
    setError(''); setInfo(''); setPw('')
    setPwFocus(false); setShowPw(false)
  ...
  async function verifyOtp() {
    ...
    if (e) {
      setOtpErr('Código inválido ou expirado. Tente novamente.')
    } else {
  ...
  async function resendOtp() {
    ...
    if (!e) setInfo('Novo código enviado!')
  }
  ...
  return (
    <div style={{ minHeight: '100dvh', background: C.bg, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '24px 20px', fontFamily: "'Inter',sans-serif" }}>
      <style>{`
      ...
      <div style={{ width: '100%', maxWidth: 400, animation: 'fIn 0.35s ease' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10, marginBottom: 28 }}>
          <div style={{ width: 52, height: 52, borderRadius: 14, background: C.accentBg, border: `1px solid ${C.accentB}`, display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: `0 0 24px ${ACCENT}25` }}>
            <ClaramenteLogo size={28} accent={ACCENT} />
          </div>
          <div style={{ textAlign: 'center' }}>
            <p style={{ fontSize: 18, fontWeight: 700, color: C.text, letterSpacing: -0.5, margin: 0 }}>Claramente</p>
            <p style={{ fontSize: 12, color: C.textMuted, marginTop: 2 }}>Sua mente em equilíbrio.</p>
          </div>
        </div>

        {/* ── OTP step ── */}
        {step === 'otp' && (
          <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 14, padding: 28, boxShadow: isDark ? '0 4px 24px rgba(0,0,0,0.4)' : '0 4px 24px rgba(37,99,235,0.08)' }}>
            <div style={{ textAlign: 'center', marginBottom: 24 }}>
              <div style={{ width: 48, height: 48, borderRadius: 12, background: C.accentBg, border: `1px solid ${C.accentB}`, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px' }}>
                <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={ACCENT} strokeWidth="1.5" strokeLinecap="round">
                  <rect x="2" y="4" width="20" height="16" rx="3"/>
                  <path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7"/>
                </svg>
              </div>
              <p style={{ fontSize: 15, fontWeight: 700, color: C.text, marginBottom: 6 }}>Verifique seu e-mail</p>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div>
                <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: C.text, marginBottom: 8, textAlign: 'center' }}>
              ...
              {otpErr && (
                <div style={{ padding: '10px 12px', borderRadius: 8, background: C.errBg, border: `1px solid ${C.errB}`, fontSize: 13, color: C.errT }}>
                  {otpErr}
                </div>
              )}
              {info && (
                <div style={{ padding: '10px 12px', borderRadius: 8, background: C.okBg, border: `1px solid ${C.okB}`, fontSize: 13, color: C.okT }}>
                  {info}
                </div>
              )}
              ...
              <button
                ...
                style={{ width: '100%', height: 42, borderRadius: 8, border: 'none', background: (otpBusy || otp.length !== 6) ? C.accentBg : ACCENT, color: (otpBusy || otp.length !== 6) ? ACCENT : '#FFF', fontSize: 14, fontWeight: 600, fontFamily: "'Inter',sans-serif", cursor: (otpBusy || otp.length !== 6) ? 'not-allowed' : 'pointer', transition: 'all 0.15s' }}>
                {otpBusy ? 'Verificando...' : 'Confirmar código'}
              </button>

              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <button onClick={resendOtp} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 12, color: ACCENT, fontFamily: "'Inter',sans-serif", padding: 0 }}>
                  Reenviar código
                </button>
                <button onClick={() => { setStep('form'); setOtp(''); setOtpErr('') }} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 12, color: C.textMuted, fontFamily: "'Inter',sans-serif", padding: 0 }}>
                  ← Voltar
                </button>
              </div>
            </div>
        ...
        {step === 'form' && (
          <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 14, overflow: 'hidden', boxShadow: isDark ? '0 4px 24px rgba(0,0,0,0.4)' : '0 4px 24px rgba(37,99,235,0.08)' }}>
            ...
            {tab !== 'forgot' && (
              <div style={{ display: 'flex', borderBottom: `1px solid ${C.border}` }}>
                {(['login', 'signup'] as Tab[]).map(t => (
                  <button key={t} onClick={() => switchTab(t)} style={{ flex: 1, height: 44, border: 'none', cursor: 'pointer', fontFamily: "'Inter',sans-serif", fontSize: 13, fontWeight: tab === t ? 600 : 400, color: tab === t ? ACCENT : C.textMuted, background: tab === t ? C.surface : C.tabBg, borderBottom: tab === t ? `2px solid ${ACCENT}` : '2px solid transparent', transition: 'all 0.15s' }}>
            ...
            <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              {tab !== 'forgot' && (
                <div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                    ...
                    {tab === 'login' && (
                      <button onClick={() => switchTab('forgot')} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 12, fontWeight: 500, color: ACCENT, fontFamily: "'Inter',sans-serif", padding: 0 }}>Esqueceu?</button>
                    )}
                  </div>
              ...
              {error && <div style={{ padding: '10px 12px', borderRadius: 8, background: C.errBg, border: `1px solid ${C.errB}`, fontSize: 13, color: C.errT }}>{error}</div>}
              {info  && <div style={{ padding: '10px 12px', borderRadius: 8, background: C.okBg,  border: `1px solid ${C.okB}`,  fontSize: 13, color: C.okT }}>{info}</div>}

              <button onClick={submit} disabled={busy} style={{ width: '100%', height: 42, borderRadius: 8, border: 'none', background: busy ? C.accentBg : ACCENT, color: busy ? ACCENT : '#FFF', fontSize: 14, fontWeight: 600, fontFamily: "'Inter',sans-serif", cursor: busy ? 'not-allowed' : 'pointer', transition: 'all 0.15s', marginTop: 2 }}
